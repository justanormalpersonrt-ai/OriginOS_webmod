...
function removeEventListener_setup() {
    ...
    setTimeout(() => {
        showPopup2_alert(
            `Low-performance detected, recommend reboot for a better experience`,
            `Yes`,
            `No`,
            () => {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (localStorage.getItem("always_on_displays_saved") == 0) {
        ...
        document.getElementById("setting-item-hide-wall").style.filter = " brightness(0.7)";
        document.getElementById("setting-item-hide-wall").style.pointerEvents = "none";
    ...
    if (saved_btn != null) {
        if (saved_btn == 1) {
            ...
            document.getElementById("btn1").style.boxShadow = "0 0 10px rgba(246, 82, 104, 0.5)";
            ...
            document.getElementById("btn2").style.boxShadow = "0 0 10px rgba(246, 82, 104, 0.5)";
    ...
    if (scale_icon) {
        ...
        document.getElementById(`scale-button${scale_icon}`).classList.add("active");
    }
    ...
    if (savedBorder) {
        ...
        if (savedBorderColor) {
            updateWallpaperBorderColor(getReadableColor(savedBorderColor), savedBorderColor);
        }
    ...
    const current_wallpaper_lock_saved = localStorage.getItem("current_wallpaper_lock");
    if (current_wallpaper_lock_saved) current_wallpaper_lock = current_wallpaper_lock_saved;
    ...
    const savedSpeed = parseFloat(localStorage.getItem("selectedLottieSpeed")) || 1;
    ...
    let volume_click_volume_saved = parseFloat(localStorage.getItem("volume_click_volume"));
    if (volume_click_volume_saved == 0) volume_click_volume = 0;

    let volume_unlock_volume_saved = parseFloat(localStorage.getItem("volume_unlock_volume"));
    if (volume_unlock_volume_saved == 0) volume_unlock_volume = 0;

    let volume_charge_volume_saved = parseFloat(localStorage.getItem("volume_charge_volume"));
    if (volume_charge_volume_saved == 0) volume_charge_volume = 0;

    if (volume_click_volume_saved > 0) toggle_click_volume.classList.add("active");
    else {
        phone.removeEventListener("click", clickSound);
        toggle_click_volume.classList.remove("active");
    }
    if (volume_unlock_volume_saved > 0) toggle_unlock_volume.classList.add("active");
    else toggle_unlock_volume.classList.remove("active");
    if (volume_charge_volume_saved > 0) toggle_charge_volume.classList.add("active");
    ...
    if (savedAnim && savedGroup) {
        ...
        const activeItem = document.querySelector(`[data-anim="${savedAnim}"][data-group="${savedGroup}"]`);
        if (activeItem) {
    ...
    if (localStorage.getItem("transformControlCenter")) {
        ...
        document
        .getElementById(`item_anim_controlsCenter_Anim${localStorage.getItem("transformControlCenter")}`)
    ...
    borderRadiusSlider.value = parseFloat(localStorage.getItem("borderRadiusSystem")) || 25;

    if (localStorage.getItem("parallelAnimation_saved") == "0") {
        document.getElementById("parallelAnimationToggle").classList.remove("active");
    ...
    if (localStorage.getItem("multipleClickApp_saved") == "0") {
        document.getElementById("multipleClickAppToggle").classList.remove("active");
    ...
    if (localStorage.getItem("timeHidingIcon")) {
        ...
        root.style.setProperty("--bg--timeHidingIcon", `${savedTime * currentSpeed}s`);
    ...
    if (localStorage.getItem("delayHidingIcon")) {
        ...
        root.style.setProperty("--bg--delayHidingIcon", `${savedDelay * currentSpeed}s`);
    ...
    if (localStorage.getItem("timeShowingIcon")) {
        ...
        root.style.setProperty("--bg--timeShowingIcon", `${savedTime * currentSpeed}s`);
    ...
    if (localStorage.getItem("delayShowingIcon")) {
        ...
        root.style.setProperty("--bg--delayShowingIcon", `${savedDelay * currentSpeed}s`);
